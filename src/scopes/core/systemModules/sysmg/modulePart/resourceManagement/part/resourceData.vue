<template>
  <div id="restUrlSelectTagContainer">
    <ta-form :auto-form-create="(form)=>{this.form = form}" :layout="formLayout">
      <template v-for="formSetting in formNormalSettings">
        <renderFormItem :key="formSetting.id" :form-setting="formSetting" :is-show-parent-item="isShowParentItem" :render-type="renderType" :render-prop="renderProp">
          <iconFormItem v-if="formSetting.id == 'icon'" />

          <rest-url-select-tag
            v-if="formSetting.id == 'restUrl'"
            new-label="添加服务"
          />
          <ta-radio-group v-else-if="formSetting.id == 'resourceType'" button-style="solid">
            <ta-radio-button
              v-for="item in resourceTypeEditable"
              :value="item.value"
              :key="item.value"
          ...
          <ta-switch v-else-if="formSetting.id == 'effectiveFlag'" checked-children="有效" un-checked-children="无效" />
          <!--<div class="menu-image" v-if="formSetting.id == 'menuImage'">
            <div v-if="imageUrl" class="menu-image-show" :style="{ backgroundImage: 'url('+imageUrl+')'}" />
          ...
          <ta-color-picker v-else-if="formSetting.id == 'iconColor'" vertical-position="top" />
        </renderFormItem>
      </template>
        ...
        <ta-collapse-panel header="更多功能信息" key="1" style="border:none">
          <template v-for="formSetting in formMoreSettings">
            <renderFormItem :key="formSetting.id" :form-setting="formSetting" :render-type="renderType" :render-prop="renderProp">
              <ta-color-picker v-if="formSetting.id == 'iconColor'" vertical-position="top" />
            </renderFormItem>
          </template>
        </ta-collapse-panel>
      </ta-collapse>
...
import iconFormItem from '@/scopes/core/systemModules/sysmg/modulePart/resourceManagement/components/iconFormItem'
...
export default {
  name: 'ResourceData',
  components: { renderFormItem, restUrlSelectTag, iconFormItem, },
  mixins: [mixins],
  props: {
    initData: {
      type: Object,
      ...
      this.form.validateFieldsAndScroll((err, values) => {
        if (!err) {
          ...
          api.saveResource({
            ...
            if (!next) {
              this.$emit('close', data.result)
            } else {
              this.$emit('next', data.result)
            }
          })
        }
      })
    ...
    fnLoadResourceData (data) {
      ...
      if (this.operationType === '2') {
        ...
        this.$nextTick(function () {
          ...
          this.form.setFieldsValue({ icon: data.taResource.icon, })
        })
