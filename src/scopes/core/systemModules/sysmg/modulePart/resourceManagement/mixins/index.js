  ...
  created () {
    if (!window.mixinsResource) {
      ...
      rapi.queryResourceSettingTable((data) => {
        if (isArray(data.resultData) && data.resultData.length > 0) {
          for (let i = 0; i < data.resultData.length; i++) {
            if (data.resultData[i].formId === 'icon') {
              data.resultData[i].formType = 'slot'
              break
            }
          }
          this.extendSettings = data.resultData
        }
    ...
    getFormNecessarySettings (initData) {
      return [
        ...
        },
        {
          ...
          label: '排序号',
          decoratorOptions: {
            rules: [{ required: true, message: '排序号不能为空!'}, { type: 'number', }],
          },
          display: true,
          exist: true,
          formItemLayout: formItemLayouts.default,
