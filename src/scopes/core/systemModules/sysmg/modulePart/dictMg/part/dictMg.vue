<template>
  <div id="dictMg" class="fit">
    <ta-border-layout
      ...
      :center-cfg="{showBar: true, toolBarStyle:{height: '50px', lineHeight: '48px'}}"
    ...
    >
      ...
      <div slot="centerExtraContent">
        <div style="float: right">
          <ta-button type="primary" @click="fnToAddDictType">
            新增类型
          </ta-button>
          <ta-dropdown>
            <ta-menu slot="overlay">
              <ta-menu-item :disabled="btnDisable">
                <ta-popconfirm title="确认启用所选字典类型?" cancel-text="取消" ok-text="确认" @confirm="fnStartDictByTypes(false)">
              ...
              <ta-menu-divider />
              <ta-menu-item :disabled="btnDisable">
                <ta-popconfirm title="确认禁用所选字典类型?" cancel-text="取消" ok-text="确认" @confirm="fnStopDictByTypes(false)">
                  <ta-icon type="stop" />
          ...
          <ta-button @click="fnRefreshCaches">
            刷新缓存
          </ta-button>
        </div>
      </div>
    ...
    >
      <ta-form :auto-form-create="(form)=>{this.form = form}">
        <ta-form-item
          ...
          :init-value="typeFormData.type"
          :field-decorator-options="{ rules: [{ required: true, message: '请输入字典类型' }]}"
        >
          <ta-input />
        </ta-form-item>
        <ta-form-item
          ...
          :init-value="typeFormData.name"
          :field-decorator-options="{ rules: [{ required: true, message: '请输入字典名称' }]}"
        >
          <ta-input />
        </ta-form-item>
        <ta-form-item
          ...
          :label-col="formItemLayout.labelCol"
          :wrapper-col="formItemLayout.wrapperCol"
          field-decorator-id="authority"
          :init-value="typeFormData.authority"
          :field-decorator-options="{ rules: [{ required: true, message: '请选择所属权限标识' }]}"
        >
          <ta-select placeholder="请选择权限标识">
            <ta-select-option v-for="item in authorityList" :key="item.value">
              {{ item.label }}
            </ta-select-option>
          </ta-select>
      ...
      <div slot="footer">
        <ta-button-group>
          ...
          <ta-button type="primary" @click="saveType">
            保存
          </ta-button>
        </ta-button-group>
      </div>
    ...
    <ta-drawer
      ...
      closable
      footer-height
      @close="fnDictAddOrEditDrawerClose"
      :visible="dictAddOrEditVisible"
    >
      <div slot="footer">
        <ta-button-group>
          ...
          <ta-button type="primary" @click="saveDict">
            保存
          </ta-button>
        </ta-button-group>
      </div>
      <system-dict-mg
        :dict="dict"
        ref="sysDictMgChild"
        @closeSystemDictMgDrawer="fnDictAddOrEditDrawerClose"
      />
    </ta-drawer>
    <ta-careful-delete :visible="deleteVisible" @close="deleteVisible = false" @delete="fnDeleteDictByTypes" title="字典类型删除" description="所选字典类型" />
  </div>
</template>
<script>
