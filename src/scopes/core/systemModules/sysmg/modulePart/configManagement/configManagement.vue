            ...
            <ta-border-layout  :showBorder="false" :centerCfg="{showBar: true, toolBarStyle:{height: '50px', lineHeight: '48px'}}">
                ...
                <ta-form :autoFormCreate="(form)=>{this.form = form}">
                    <ta-table
                            ...
                            @expand="loadChild"
                            :scroll="{ x: 1200 }">
                        <span slot="fieldName"  slot-scope="text" >{{text}}</span>
...
const columns = [
  { title: '字段名', dataIndex: 'fieldName', overflowTooltip: true, width: '360px', scopedSlots: { customRender: 'fieldName', }, },
...
export default {
  ...
  components: { TaTextarea, moment, addConfig, },
  mounted () {
    this.initForm()
  },
  methods: {
    ...
    changeData: function ({ newData, record, columnKey, }, callback) {
      this.$confirm({
        title: '确定修改配置？',
        onOk: () => {
