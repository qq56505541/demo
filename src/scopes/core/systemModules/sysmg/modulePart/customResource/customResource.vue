  ...
  <div id="app" class="fit">
    <ta-border-layout :layout="{left:'300px'}" :center-cfg="{layoutConStyle:{padding:0,border: 0}}">
      <div slot="left" class="fit">
        <ta-card :bordered="false" style="width: 100%" title="资源类别" :body-style="{'padding':0}">
      ...
      <ta-border-layout  :show-border="false" :center-cfg="{showBar: true, toolBarStyle:{height: '50px', lineHeight: '48px'}}">
        <div slot="header" class="center-box">
          <ta-input-search v-model="param" placeholder="输入资源名称/自定义编码/所属系统" enter-button="搜索" class="search-box" @search="queryAllCustomResource" />
        </div>
        <div slot="centerExtraContent" style="float: right">
          <ta-button type="primary" class="step1" @click="showCatagoryDrawer()">
            新增授权对象类型
          </ta-button>
        </div>
        <ta-table
    ...
    >
      <ta-form :auto-form-create="(form)=>{this.form = form}">
        <ta-form-item
          ...
          field-decorator-id="customResourceId"
          :field-decorator-options="{ initialValue: formData.customResourceId }"
          style="display: none;"
        >
          <ta-input />
        </ta-form-item>
        <ta-form-item
          ...
          :label-col="labelCol"
          :wrapper-col="wrapperCol"
          field-decorator-id="resourceName"
          :field-decorator-options="{ initialValue: formData.resourceName, rules: [{ required: true, message: '请输入资源名称' },{max:450,message:'不能超过450个字符'}]}"
        >
        ...
        <ta-form-item
          ...
          :label-col="labelCol"
          :wrapper-col="wrapperCol"
          field-decorator-id="code"
          :field-decorator-options="{ initialValue: formData.code, rules: [{ required: true, message: '请输入自定义编码' },{max:100,message:'不能超过100个字符'},{pattern:new RegExp('^[a-z0-9]*$'),message:'输入格式错误'}]}"
        >
        ...
        <ta-form-item
          ...
          :label-col="labelCol"
          :wrapper-col="wrapperCol"
          field-decorator-id="resourceCategory"
          :field-decorator-options="{ initialValue: formData.resourceCategory, rules: [{ required: true, message: '请选择所属类别' }]}"
        >
          <ta-select allow-clear placeholder="请选择所属类别" disabled>
            <ta-select-option v-for="item in selectData" :key="item.id">
              {{ item.name }}
            </ta-select-option>
          </ta-select>
        </ta-form-item>
        <ta-form-item
          ...
          :label-col="labelCol"
          :wrapper-col="wrapperCol"
          field-decorator-id="parentId"
          :field-decorator-options="{ initialValue: formData.parentId, rules: [{ required: true, message: '请选择上级节点' }]}"
        >
          <ta-select allow-clear placeholder="请选择上级节点" :disabled="true">
            <ta-select-option v-for="item in selectParent" :key="item.id">
              {{ item.name }}
            </ta-select-option>
          </ta-select>
        </ta-form-item>
        <ta-form-item
          ...
          :label-col="labelCol"
          :wrapper-col="wrapperCol"
          field-decorator-id="resourceContent"
          :field-decorator-options="{ initialValue: formData.resourceContent }"
        >
        ...
        <ta-form-item
          ...
          :label-col="labelCol"
          :wrapper-col="wrapperCol"
          field-decorator-id="system"
          :field-decorator-options="{ initialValue: formData.system, rules: [{ required: true, message: '请选择所属系统' }]}"
        >
          <ta-select allow-clear placeholder="请选择所属系统">
            <ta-select-option v-for="item in selectSystem" :key="item.sysCode">
              {{ item.name }}
            </ta-select-option>
          </ta-select>
        </ta-form-item>
      ...
      <div slot="footer">
        <ta-button-group>
          <ta-button @click="reSetForm()">
            重置
          </ta-button>
          <ta-button type="primary" @click="saveCustomResource">
            保存
          </ta-button>
        </ta-button-group>
      </div>
    ...
    <ta-drawer
      ...
      :getContainer="false"
      @close="categoryVisible = false; form1.resetFields(); formData1 = {}"
    >
      <ta-form :auto-form-create="(form) => {this.form1 = form}">
        <ta-form-item
          ...
          :label-col="labelCol"
          :wrapper-col="wrapperCol"
          field-decorator-id="categoryId"
          :field-decorator-options="{ initialValue: formData1.categoryId }"
          style="display: none;"
        >
          <ta-input />
        </ta-form-item>
        <ta-form-item
          ...
          :label-col="labelCol"
          :wrapper-col="wrapperCol"
          field-decorator-id="categoryName"
          :field-decorator-options="{initialValue: formData1.categoryName, rules: [{ required: true, message: '请输入类别名称' },{max:10,message:'不能超过10个字符'}]}"
        >
        ...
        <ta-form-item
          ...
          :wrapper-col="wrapperCol"
          field-decorator-id="code"
          :field-decorator-options="{initialValue: formData1.code, rules: [{ required: true, message: '请输入类别编码' },{max:100,message:'不能超过100个字符'},{pattern:new RegExp('^[a-z0-9]*$'),message:'输入格式错误'}]}"
        >
        ...
        <ta-form-item
          ...
          :label-col="labelCol"
          :wrapper-col="wrapperCol"
          field-decorator-id="categoryContent"
          :field-decorator-options="{initialValue: formData1.categoryContent, rules: [{ required: true, message: '请输入类别描述' },{max:1024,message:'不能超过1024个字符'}]}"
        >
          <ta-input placeholder="请输入类别描述" />
        </ta-form-item>
        <ta-form-item label="有效标识" :label-col="labelCol" :wrapper-col="wrapperCol" field-decorator-id="effective" :field-decorator-options="{ valuePropName: 'checked', initialValue: formData1.effective}">
          <ta-switch checked-children="有效" un-checked-children="无效" @change="onchange" />
        </ta-form-item>
      </ta-form>
      <div slot="footer">
        <ta-button-group>
          ...
          <ta-button type="primary" @click="saveCatagory">
            保存
          </ta-button>
        </ta-button-group>
      </div>
    ...
    const steps = [
      {
        ...
        popover: {
          title: '新增授权对象类型',
          description: '自定义资源用于业务系统权限功能拓展，新增的自定义资源可以像普通功能资源一样被角色权限控制',
          position: 'left',
        },
      }
    ]
