<template>
    <ta-drawer title="新增系统配置" width="500" placement="right" :closable="true" @close="closeEdit" :visible="visible" destroyOnClose>
        <ta-form :autoFormCreate="(form)=>{this.form = form}">

            <ta-form-item
                    label='字段名'
                    fieldDecoratorId="fieldName"
                    :fieldDecoratorOptions="{ rules: [{ required: true, message: '字段名不能为空!' }], initialValue:''}">
                <ta-input />
            </ta-form-item>
            ...
            <ta-form-item
                    v-else-if="type == '5'"
                    label='默认值'
                    fieldDecoratorId="defaultYmlValue"
    ...
    save () {
      this.form.validateFields((err, values) => {
        ...
        if (this.type === '2') {
          ...
          data.defaultValue = data.defaultYmlValue
        }
