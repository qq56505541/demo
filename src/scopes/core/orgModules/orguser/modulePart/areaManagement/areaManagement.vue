  ...
  <div id="app" class="fit" ref="areaMg">
    <ta-border-layout  :centerCfg="{showBar: true, toolBarStyle:{height: '50px', lineHeight: '48px'}}">
      ...
      <div slot="centerExtraContent">
        <ta-tag-select title="有效性" :data="CollectionData('EFFECTIVE')" v-model="selectFilter" @change="filterClick" class="filter-name step1"></ta-tag-select>
        <div style="float: right">
          <ta-button type="primary" @click="showAddDrawer('0')" class="step2">新增顶级行政区划</ta-button>
          <!--begin 批量操作按钮-->
          <ta-dropdown>
    ...
    <ta-drawer :getContainer="false" :destroyOnClose="true" :title="editOrShowTitle" placement="right"
    ...
    <ta-drawer :getContainer="false" :destroyOnClose="true" :title="addOrChildTitle" placement="right"
...
const columns = [
  ...
  { title: '区划代码', key: 'areaCode', width: '15%', dataIndex: 'areaCode' },
  {
    title: '区划层级',
    key: 'areaLevel',
    dataIndex: 'areaLevel',
...
export default {
  ...
  mounted () {
    ...
    const steps = [
      ...
      },
      {
        ...
        popover: {
          title: '新增顶级行政区划',
          description: '创建顶级行政区划，创建成功后通过【新增下级】创建下级行政区划',
          position: 'left',
        },
      }
    ]
  ...
  methods: {
    ...
    fnBatchPickArea (record) {
      ...
      let areaIdPaths = []
      if (record) {
        if (record.effective == '1' && record.childNum == 0) {
