<template id="app">
  <div id="customOrgManagement" ref="customOrg" class="fit">
    <ta-border-layout :layout="{left:'300px'}" :center-cfg="{layoutConStyle:{padding:0,border: 0}}">
      ...
      <div slot="left">
        ...
        >
          <div slot="title">
            ...
            <div style="float: right">
              <ta-button
                ...
                @click="customOrgTypeNameVisible = true"
              >
                组织类别管理
              </ta-button>
            </div>
          ...
          <a
            v-for="(item, index) in orgTypeNameList"
      ...
      <ta-border-layout  :show-border="false" :center-cfg="{showBar: true, toolBarStyle:{height: '50px', lineHeight: '48px'}}">
        ...
        <div slot="centerExtraContent">
          ...
          />
          <div style="float: right">
            <ta-button v-if="showAddTop" type="primary" class="step2" @click="fnAddTopOrg">
              新增组织
            </ta-button>
            <ta-dropdown :trigger="['click']">
              <ta-menu slot="overlay">
                <ta-menu-item :disabled="!selectedRowKeys.length">
                  <ta-popconfirm title="确认启用所选账户?" cancel-text="取消" ok-text="确认" @confirm="fnBatchPickOrg(false)">
                ...
                <ta-menu-divider />
                <ta-menu-item :disabled="!selectedRowKeys.length">
                  <ta-popconfirm title="确认禁用所选账户?" cancel-text="取消" ok-text="确认" @confirm="fnBatchBanOrg(false)">
                    <ta-icon type="stop" />
        ...
        <ta-table
          ...
          row-key="customOrgId"
          :pagination="false"
          :default-expanded-row-keys="currentExpand"
          :expanded-row-keys="currentExpand"
    ...
    >
      <div slot="footer">
        <ta-button-group>
          ...
          <ta-button type="primary" :loading="btnLoading" @click="fnUpdateOrSaveOrg">
            保存
          </ta-button>
        </ta-button-group>
      </div>
      <add-custom-org ref="customOrgChild" :org="org" @closeCustomOrgDrawer="drawerClose" />
    </ta-drawer>
    <ta-careful-delete :visible="batchDeleteVisible" title="自定义组织删除" description="选中自定义组织" @close="batchDeleteVisible = false" @delete="fnBatchDeleteOrg(false)" />
    <ta-modal
      width="80%"
...
export default {
    ...
    const steps = [
      {
        ...
        popover: {
          title: '组织类别管理',
          description: '创建和编辑组织类别，通过组织类别管理多个自定义组织树',
          position: 'bottom',
        },
      },
      {
        ...
        popover: {
          title: '新增组织',
          description: '创建组织树，组织树创建成功之后可以通过【人员管理】进行组织人员关系绑定，通过【新增下级】创建下级组织',
          position: 'left',
        },
      }
    ]
  ...
  methods: {
    ...
    moment,
    setContainer () {
      return document.getElementById('customOrgManagement')
    },
      ...
      if (expanded) {
        ...
        if (record.children && record.children.length > 0) {
          ...
          $api.queryCustomOrgByParentId(data, (data) => {
            ...
            if (customOrgId !== undefined && customOrgId !== '') {
              // 设置默认展开的层级,默认展开第一层,显示两级
              this.currentExpand.push(customOrgId)
            } else {
              const hasData = record.children && record.children > 0
              if (hasData) {
                this.currentExpand.push(this.record.children[0].customOrgId)
              }
            }
          })
        }
    ...
    fnInitDefaultOrg (val, pid) {
      ...
      $api.queryCustomOrgByParentId(data, (data) => {
        ...
        this.$nextTick(() => {
          ...
          if (hasData) {
            if (this.orgData[0].children.length > 0) {
    ...
    fnBatchPickOrg (record) {
      const data = {}
      if (record) {
        if (record.effective == '1' && record.isLeaf) {
    ...
    drawerClose (newData) {
      ...
      // 不再手动前端构建节点，调用后台接口刷新同级节点（树状表格）或列表
      this.fnInitDefaultOrg(this.orgTypeNameSelVal, null)
      // 新增顶级组织
