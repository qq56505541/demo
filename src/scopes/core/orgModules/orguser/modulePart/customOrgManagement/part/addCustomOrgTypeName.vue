  ...
  <ta-tabs class="fit">
    ...
    <ta-tab-pane tab="自定义组织类别列表">
      ...
      <ta-table :columns="customOrgTypeNameColumns" :dataSource="customOrgTypeNameData" rowKey="customOrgTypeNameId" style="padding-top: 10px">
        <ta-table-edit :editForm="form1" slot="customOrgTypeName" slot-scope="text, record" type="input" :row-editable="true" :rules="[{validator: fnValidate}]" placeholder="请输入类别名称"></ta-table-edit>
        <ta-table-edit :editForm="form1" slot="customOrgTypeNameCode" slot-scope="text, record" type="input" :row-editable="true" placeholder="请输入类别自定义编码" :rules="[{ required: true, message: '自定义编码不能为空'}, { pattern: /^[\u4E00-\u9FA5A-Za-z0-9_/\\-]+$/, message: '只能输入中文英文数字、下划线、横杠及斜杠' }]"></ta-table-edit>
        <ta-table-edit :editForm="form1" slot="customOrgTypeNameDesc" slot-scope="text, record" type="input" :row-editable="true" placeholder="请输入类别描述"></ta-table-edit>
        <ta-table-edit :editForm="form1" slot="effective" slot-scope="text, record" type="switch" trueText="有效" falseText="无效" :row-editable="true"></ta-table-edit>
        <ta-table-edit :editForm="form1" slot="action" slot-scope="text, record" :dataSource="customOrgTypeNameData" rowKey="customOrgTypeNameId" type="rowEdit" :beforeChange="beforeChange" @tableChange="changeTable" @rowDelete="fnDelete"></ta-table-edit>
      </ta-table>
      </ta-form>
    </ta-tab-pane>
  </ta-tabs>
