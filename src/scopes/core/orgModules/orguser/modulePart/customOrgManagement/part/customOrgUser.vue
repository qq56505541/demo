<template>
  <div id="customOrgUser" class="fit">
    <ta-border-layout :layout="{header:'70px', footer: '70px'}" :center-cfg="{showBar: true}" :footer-cfg="{showBorder: false}">
      ...
      <ta-alert slot="centerExtraContent" :message="'当前组织路径：'+this.$route.query.customOrgPath" type="info" show-icon />
      <ta-tabs class="fit">
        <template slot="tabBarExtraContent">
          ...
          <ta-button-group>
            ...
            <ta-button @click="fnShowUserSelectModal" type="primary">
              分配人员
            </ta-button>
          </ta-button-group>
        </template>
    ...
    <custom-org-user-select :visible="visible" @close="fnCloseUserSelectModal" />
  </div>
</template>
