  ...
  <div id="publicRoleUser" class="fit">
    <ta-border-layout :layout="{header:'70px', footer: '70px'}" :footer-cfg="{showBorder: false}">
      <div slot="header" class="center-box">
        ...
        <div class="divider" />
        <ta-alert :message="'当前角色为：'+role.roleName" type="info" show-icon class="notice-box" />
      ...
      <ta-tabs class="fit content-box">
        ...
        <template slot="tabBarExtraContent">
          <ta-input-search
            ...
            placeholder="请输入人员姓名"
            class="search-box"
            @search="fnQueryUsersByRoleId"
          >
            <ta-button slot="enterButton" type="primary">
              搜索
            </ta-button>
          </ta-input-search>
          <ta-button type="primary" @click="fnOpenAssociate">
            可分配人员
          </ta-button>
          <ta-button @click="deleteBatchRoleUsers">
            <ta-icon type="delete" />
            批量移除
          </ta-button>
        </template>
...
export default {
  ...
  methods: {
    // 批量移除
    deleteBatchRoleUsers () {
      const table = this.$refs.table
      const { selectedRowKeys, } = table.getChecked()
      const userIds = selectedRowKeys.join(',')
      $api.deleteBatchRoleUsers({ roleId: this.role.roleId, userIds: userIds, }, (data) => {
    ...
    fnOpenAssociate () {
      ...
      $api.queryUsersByRoleIdNoPage({ roleId: this.role.roleId, }, (data) => {
        this.defaultUserList = data.data.list
      })
    },
    fnBack () {
      this.$router.push({ name: 'publicRoleManager', })
    ...
    fnSearchUser (value) {
      if (value) {
        $api.queryUsersNoWraperByRoleId({
    ...
    checkAllMembers (orgId, includeChild,cb) {
      $api.queryUsersNoWraperByRoleIdNoPage({ roleId: this.role.roleId, orgId: orgId, includeChild: includeChild,}, (data) => {
        cb(data.data.list)
      })
    },
