<template>
  <div class="fit" id="examineManagement">
    <ta-border-layout :layout="{header:'70px'}" :centerCfg="{showBar: true, toolBarStyle: {height: '50px', lineHeight: '48px'}}">
      ...
      <ta-table
        ...
        <span slot="checkStatus" >未审核</span>  <!-- 该功能查询出来都是未审核-->
    ...
    <show-custom-resource-category :visible="customResourceCategoryVisible" :rowData="rowData" :chooseCustomResourceResult="chooseCustomResourceResult" @close="customResourceCategoryVisible = false"></show-custom-resource-category>
    <show-edit-custom-resource-category :visible="customResourceCategoryEditVisible" :rowData="rowData" @close="customResourceCategoryEditVisible = false"></show-edit-custom-resource-category>

    <show-custom-resource :visible="customResourceVisible" :rowData="rowData" :chooseCustomResourceResult="chooseCustomResourceResult" @close="customResourceVisible = false"></show-custom-resource>
    <show-edit-custom-resource :visible="customResourceEditVisible" :rowData="rowData" @close="customResourceEditVisible = false"></show-edit-custom-resource>
...
import showCustomResourceCategory from './part/customResource/showCustomResourceCategory'
import showEditCustomResourceCategory from './part/customResource/showEditCustomResourceCategory'

import showCustomResource from './part/customResource/showCustomResource'
import showEditCustomResource from './part/customResource/showEditCustomResource'
...
export default {
  ...
  data () {
    return {
      ...
      customResourceCategoryVisible: false,
      customResourceVisible: false,
      customResourceEditVisible: false,
      customResourceCategoryEditVisible: false,
      chooseResult: '',
      chooseUserResult: '',
      chooseRoleResult: '',
      chooseCustomResourceResult: '',
      adminRole: false,
  ...
  components: {
    ...
    showCustomResourceCategory,
    showEditCustomResourceCategory,
    showCustomResource,
    showEditCustomResource,
  },
  mounted () {
    this.fnQuery()
  },
  methods: {
    ...
    showDetail (record) {
      ...
      } else if (record.opObjectType === 'CUSTOM_RESOURCE') {
        if (record.opName.indexOf('自定义资源类别') !== -1) {
          if (record.opName.indexOf('新增') !== -1) {
            this.customResourceCategoryVisible = true
            this.chooseCustomResourceResult = 'after'
          } else if (record.opName.indexOf('更新') !== -1) {
            this.customResourceCategoryEditVisible = true
          } else if (record.opName.indexOf('删除') !== -1) {
            this.customResourceCategoryVisible = true
            this.chooseCustomResourceResult = 'before'
          } else {
            this.customResourceCategoryVisible = true
            this.chooseCustomResourceResult = 'after'
          }
        } else if (record.opName.indexOf('自定义资源') !== -1) {
          if (record.opName.indexOf('新增') !== -1) {
            this.customResourceVisible = true
            this.chooseCustomResourceResult = 'after'
          } else if (record.opName.indexOf('更新') !== -1) {
            this.customResourceEditVisible = true
          } else if (record.opName.indexOf('删除') !== -1) {
            this.customResourceVisible = true
            this.chooseCustomResourceResult = 'before'
          } else {
    ...
    getContainer () {
      return document.getElementById('examineManagement')
    },
  },
}
</script>
