<template>
  <div id="staticResourceMg" class="fit">
    <ta-border-layout
      :layout="{left:'300px'}"
      :left-cfg="{title:'资源管理',showBar:true}"
    ...
    >
      ...
      <ta-border-layout
        ...
        :center-cfg="{showBorder:false, showBar: true, toolBarStyle:{height: '50px', lineHeight: '48px'}}"
      >
        ...
        <div slot="centerExtraContent">
          ...
          <div style="float: right">
            ...
            <ta-button-group>
              <ta-button @click="fnShowAddDirectory">
                新建目录
              </ta-button>
              <ta-button @click="fnShowAddFile">
                新建文件
              </ta-button>
            </ta-button-group>
          </div>
    ...
    <ta-modal
      ...
      destroy-on-close
    >
      <ta-form :auto-form-create="(form)=>{this.addDirectoryForm = form}">
        <ta-alert :message="'当前所在目录：'+ currentDirectory" type="info" show-icon class="message-box" />
        <ta-form-item
          label="文件夹名称"
          :label-col="{span:6}"
          :wrapper-col="{span:18}"
          field-decorator-id="fileName"
          :field-decorator-options="{rules: [{ required: true,message:'文件夹名为必输'}]}"
        >
          <ta-input placeholder="请输入目录名称，可建多级目录，用/分隔" />
        </ta-form-item>
      </ta-form>
    </ta-modal>
    <ta-modal
      ...
      destroy-on-close
    >
      <ta-form :auto-form-create="(form)=>{this.addFileForm = form}">
        <ta-alert :message="'当前所在目录：'+ currentDirectory" type="info" show-icon class="message-box" />
        <ta-form-item
          label="文件名称"
          :label-col="{span:6}"
          :wrapper-col="{span:18}"
          field-decorator-id="fileName"
          :field-decorator-options="{rules: [{ required: true,message:'文件名为必输'}]}"
        >
          <ta-input placeholder="请输入文件名称，可在多级子目录下建立文件，用/分隔" />
        </ta-form-item>
        <ta-form-item
          label="文件内容"
          :label-col="{span:6}"
          ...
          :field-decorator-options="{rules: [{ required: true,message:'文件内容为必输'}]}"
        >
          <ta-textarea style="height: 150px;" />
        </ta-form-item>
      </ta-form>
    </ta-modal>
  </div>
  ...
  methods: {
    ...
    uploadData (file) {
      return {
        fileName: file.name,
        filePath: this.currentDirectory,
        ...TaUtils.getProperty('TA$indexParam', {}),
      }
    },
  },
  activated () {
