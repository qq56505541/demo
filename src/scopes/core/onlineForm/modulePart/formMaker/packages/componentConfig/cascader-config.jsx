    ...
    return (
      <base-config
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <ta-label-con label="展开方式" style="margin-top:18px">
            <ta-select vModel={this.config.expandTrigger} style="width: 100%">
              <ta-select-option value="click">鼠标点击</ta-select-option>
              <ta-select-option value="hover">鼠标移入</ta-select-option>
            </ta-select>
          </ta-label-con>
          <ta-label-con label="清除按钮">
            <ta-switch vModel={this.config.allowClear}/>
          </ta-label-con>
          <ta-label-con label="点击即改变">
            <ta-switch vModel={this.config.changeOnSelect}/>
          </ta-label-con>
          <ta-modal
            visible={this.editModalVisible}
            title={this.modalTitle}
            height={this.operateType === 'edit' ? '110px' : '160px'}
            ...
            <ta-form autoFormCreate={(form) => {
              ...
              {['add', 'addRoot'].includes(this.operateType) && <ta-form-item
                label="value"
