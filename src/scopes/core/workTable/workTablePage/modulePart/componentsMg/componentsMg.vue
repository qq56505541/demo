<template>
  <div class="fit">
    ...
    >
      ...
      <div style="padding-left: 10px;padding-right: 10px;margin-bottom: 10px;margin-top: 5px;">
        ...
        <div style="float: right;">
          <ta-button type="primary" @click="fnOnAddClick">
            新增
          </ta-button>
          <ta-dropdown>
            <ta-menu slot="overlay">
              <ta-menu-item :disabled="!selectedKeys.length">
                <ta-popconfirm title="确认启用所选工作台组件?" cancel-text="取消" ok-text="确认" @confirm="fnUpdateStatus(false, '1')">
              ...
              <ta-menu-divider />
              <ta-menu-item :disabled="!selectedKeys.length">
                <ta-popconfirm title="确认禁用所选工作台组件?" cancel-text="取消" ok-text="确认" @confirm="fnUpdateStatus(false, '0')">
                  <ta-icon type="stop" />
      ...
      <ta-table ref="componentTable" :columns.sync="componentColumns" :data-source="componentData" row-key="resourceId" :pagination="false" :row-selection="{selectedRowKeys: selectedKeys, onChange: handleSelectChange}">
      ...
      <div slot="footer">
        <ta-pagination
          ...
          show-size-changer
          show-quick-jumper
          :data-source.sync="componentData"
    ...
    <componentEditDrawer
      ...
      :form-init-value="formInitValue"
      @onSaveClick="fnOnSave"
    />
    <ta-careful-delete :visible="deleteVisible" @close="deleteVisible = false" @delete="fnOnDeleteClick" title="工作台组件删除" description="选中的工作台组件" />
  </div>
</template>
...
export default {
  name: 'ComponentsMg',
  components: { componentEditDrawer, TaTable, },
  data () {
