<template>
  <div class="page">
    <work-table
      ...
      :editable="editable"
      :type="type"
      :is-role-template="isRoleTemplate"
    ...
    <switchModal
      ...
      :template-selected-tag.sync="templateSelectedTag"
      @generateWorktable="generateWorktable"
    />
    <ta-page-tool :tool-menu="toolMenu" :is-move="true" :is-refresh="false" />
  </div>
</template>
...
export default {
  ...
  methods: {
        ...
        this.$confirm({
          ...
          onOk: () => {
            api.saveRoleWorkbench({ roleId: this.roleId, workbenchStr: JSON.stringify(this.layout), }, res => {
              ...
              window.parent.indexTool.closeTabMenu('roleWorktableTemplateModify')
              window.parent.indexTool.openTabMenu({ id: '0aac95c1e73947bea41be639cc4e9036', exist: true, })
              this.editable = false
              this.showAll = false
            })
          },
    ...
    onCkbgChange: function (newChecked) {
      ...
      const temp = this.getArrDifference(this.checked, newChecked)
