  ...
  <div class="formWork">
    <transition name="operateTools">
      <div v-show="editable" class="operateTool">
        <ta-button class="operateBtn swap" :disabled="type==='1'" @click="$emit('swap')" icon="swap">
          切换工作台
        </ta-button>
        <ta-button v-show="!isRoleTemplate" class="operateBtn save" @click="$emit('save')">
          保存
        </ta-button>
        <ta-button v-show="!isRoleTemplate" class="operateBtn" @click="$emit('add')">
          新增
        </ta-button>
        <ta-button class="operateBtn" @click="$emit('cancel')">
          取消
        </ta-button>
      </div>
    </transition>
