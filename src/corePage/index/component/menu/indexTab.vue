  ...
  <div class="fit">
    <div v-if="state.barType === 'tab'" class="index-tabs" ref="index-tabs">
      <div
        class="worktable-tab"
        id="worktable"
        @click="setTabsted({id:'worktable'})"
        :title="worktableName"
        ref="index-worktable"
        :class="{active:activeTab==='worktable'}"
      >
        {{ worktableName }}
      </div>
      <div
          ...
          >
            ...
            <span flag="tab-more">
              <ta-icon
                ...
                type="activeTab==menu.id?'close-circle':close"
                @click.stop="closeTabFn(menu)"
              />
            </span>
          </div>
          <div class="close-all" flag="tab-more" @click="closeAllTab()">
            {{$t('workTable.closeAll')}}
          </div>
        </div>
      </div>
    </div>
    <ta-breadcrumb v-else class="index-breadcrumb">
      ...
      <ta-breadcrumb-item v-for="(item, index) in state.breadcrumb" :key="index">
        {{ item }}
      </ta-breadcrumb-item>
    </ta-breadcrumb>
  </div>
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      getTabMenuByUrl: 'getTabMenuByUrl',
      getActiveIframe: 'getActiveIframe',
    }),
    activeTab () {
    ...
    worktableName () {
      ...
      return worktable.name || this.$t('workTable.workTable')
    },
  },
  mounted: function () {
  ...
  methods: {
    ...
    setTab () {
      if (this.state.barType === 'tab') {
        ...
        this.list2 = []
        const conL = TaUtils.getWidth(this.$refs['index-tabs'])// 容器宽度
        let tabsL = parseInt(TaUtils.getWidth(this.$refs['index-worktable'])) + 80// 工作台+更多空白
        const items = this.tabList
        const L1 = []
        const L2 = []
        this.$nextTick(function () {
          if (this.activeTab !== 'worktable') {
            tabsL = tabsL + TaUtils.getWidth(document.getElementById(this.activeTab + '-tab'))
          }
          for (let i = 0; i < items.length; i++) {
            ...
            tabsL += TaUtils.getWidth(document.getElementById(items[i].id + '-tab'))
            if (conL > tabsL) {
              L1.push(items[i])
            } else {
              L2.push(items[i])
    ...
    // 如果menu有回调的话 那么他们自己去控制是否关闭页面
    closeTabFnCheck (menu) {
      const modules = this.state.closeTabCallBackModules
      for (let i = 0; i < modules.length; i++) {
        if (modules[i]?.id === menu.id || (modules[i]?.url === menu.url && (!modules[i].prefix || modules[i].prefix === menu.prefix))) {
          const module = this.getTabMenuByUrl(menu)
          const id = module.prefix + module.module
          const pageObj = document.getElementById(id).contentWindow.pageVmObj
          pageObj.$bus.emit('closeTabCallBack', module)
          return false
        }
      }
      return true
    ...
    // 设置关闭事件
    closeTabFn (menu, force) {
      const tempModule = this.menuToModule(menu)
      if (tempModule.module !== this.getActiveIframe.module) {
        sendMessage(this.getActiveIframe.module, 'loadCachedRouter')
      }
      sendMessage(tempModule.module, 'deleteCachedRouter', tempModule.partId)
      if (!force) {
        if (!this.closeTabFnCheck(menu)) {
          return false
        }
      }
      // 找到index
  ...
  .index-tabs {
    ...
    > .worktable-tab, > .tab-item {
      ...
      .btn-close { margin-left: 10px; color: @text-color-secondary; display: block; float: left; width: 12px; height: 39px;
        i { vertical-align: 0; }
        /deep/svg { width: 10px; height: 10px; }
        &:hover {
          .close-icon-style
        }
      }
