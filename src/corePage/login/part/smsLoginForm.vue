<template>
  <ta-form :auto-form-create="(form)=>{this.form = form}" layout="vertical">
    <ta-form-item
      ...
      :field-decorator-options="{rules: [{ required: true, message: '请输入手机号!' }]}"
    >
    ...
    <ta-form-item
      ...
      :field-decorator-options="{rules: [{ required: true, message: '请输入验证码!' }]}"
    >
    ...
    <ta-form-item>
      <ta-button :disabled="disableSubmitButton" id="sbbtn" type="primary" block @click="handleSubmit" @keyup.enter="handleSubmit">
        立即登录
      </ta-button>
    </ta-form-item>
  </ta-form>
