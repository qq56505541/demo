...
(function ($) {
  ...
  function slideCheckCode (id, options) {
    const $p = $('#' + id)
    let mouseDragStartX = 0// 每次记录事件的开始X坐标轴 单位px
    let mouseDragEndX = 0// 每次记录事件的结束X坐标轴 单位px
    let isDown = false// 记录当前拖动是否已经完成
    let isCheck = false// 记录校验标志
    let isRefresh = true; let isClickRefresh = false// 刷新标志isRefresh 控制是否重新计算图片的可拖拽范围（需保证前后台配置图片宽度一致）;isClickRefresh 处理特殊情况，见代码
    let mouseDragNum = 0// 记录总共移动的距离 单位px
    let mouseDragNumImg = 0
    let dragStartDate
    let dragEndDate
    let maxDragSize = 0// 记录dragBar可拖动的最大距离
    let winMaxDragSize = 0// 记录图片可拖动的范围，实际可拖动的最大距离还要减去拖动图片自身的宽度
    ...
    function init () {
      ...
      $(document).mousemove(function (e) {
        if (isDown) {
          ...
          let dragX = mouseDragEndX - mouseDragStartX
          // 拖动范围控制
          if (dragX < 0) dragX = 0
    ...
    function dargMove (dargSize) {
      const ratio = dargSize / maxDragSize// 计算偏移比例
    ...
    function refreshSlideCheckCode () {
      Base.submit(null, { url: '/captcha/slide/getSlideParamImg', }, {
        successCallback: (data) => {
          // 背景图获取
          $('.codeDrag-code-img', $p).attr('src', faceConfig.basePath + '/captcha/slide/getSlideBackGroundImg?ticket=' + data.data.ticket)
          // 拖动图获取
          $('.codeDrag-darg-img', $p).attr('src', faceConfig.basePath + '/captcha/slide/getSlideDragImg?ticket=' + data.data.ticket)
        },
      })
    }
    // /**
    ...
    //   Base.submit(null, { url: '/captcha/slide/verifySlide',
    //     data: {
    //       slidetime: slidetime,
