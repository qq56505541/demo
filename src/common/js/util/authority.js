...
Vue.prototype.checkRole = (_vm) => {
  try {
    ...
    const moduleId = TaUtils.getNowPageParam()._modulePartId_
    if (!moduleId) {
      return
    }
    ...
    const defaultAuth = auth.defaultAuth; const list = auth.list; let authFlag = defaultAuth
    // 如果有id并且在权限list中找到对应的控制那么走权限控制auth否则走defaultAuth
    const id = _vm.$attrs.id
